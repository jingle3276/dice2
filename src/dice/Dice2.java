...
public class Dice2 {
	 ...
	 * Question 1:
	 * Roll N: 
	 * Write a function called rollN that takes a single integer parameter N and returns an array 
	 * of N integers where the value of each integer is (randomly) between one and six. 
	 * You should call a standard random number generator to produce the values between
	 ...
	 * In some cases, there might be a tie for the most frequent sum hence the need to return an 
	 * array of integers. One of the test cases will call mostFrequentSum with 5 as a parameter.   
	 */
	...
	static int[] findMostFequentinTable(Hashtable<Integer, Integer> table){
		...
		Queue<Entry<Integer, Integer>> maxQ = new PriorityQueue<Entry<Integer, Integer>>(
		...
		int max = maxQ.remove().getKey();
		list.add(max);
		while(!maxQ.isEmpty() && max == maxQ.peek().getKey()){
			list.add(maxQ.remove().getKey());
		}
	 ...
	 * Question 3:
	 * One to one hundred
	 * Write a function called one2oneHundred that takes as input an integer array of length 5 
	 * where each element is a number between 1 and 6 inclusive.	
     ...
     * distributed between 1 and 100. You might not be able to get a very even distribution
     *  - but get as close as you (reasonably) can.
	 * Essentially generate a random number in the range 1 to 100 inclusive where the only source 
	 * of randomness is the (simulated) roll of five dice
	 */
	static int one2oneHundred(int[] dice) {
		//?? can I use built-in random function ?
		int result = 0;
